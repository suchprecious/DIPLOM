...
class FeatureEngineer:
    """
    Инженерия признаков для модели рекомендации технологий
    Объединяет текстовые эмбеддинги, категориальные признаки и сущности
    """
    ...
    def __init__(self):
        ...
        self.entity_feature_dim = 6  # Количество фичей из entity extractor
    ...
    def fit(self, categorical_data: pd.DataFrame) -> None:
        """
        Обучение препроцессора на категориальных данных
        
        Args:
            categorical_data: DataFrame с категориальными признаками
        """
        ...
        try:
            ...
            # Создаем и обучаем препроцессор (исправленная версия для новых sklearn)
    ...
    def create_feature_vector(self, 
        ...
        Args:
            ...
            categorical_data: Категориальные признаки проекта
            entity_features: Извлеченные сущности из текста
        ...
        # Текстовые эмбеддинги (обязательные)
        if text_embedding is not None and text_embedding.size > 0:
            # Если это 1D массив, преобразуем в 2D
        ...
        # Категориальные признаки (опциональные)
        ...
        # Объединение с проверкой размерностей
        feature_vector = self._safe_hstack(features_list)
        
        # Сохраняем ожидаемую размерность для предсказаний
    ...
    def _process_entity_features(self, entity_features: np.ndarray) -> Optional[np.ndarray]:
        """Обработка entity features"""
        if entity_features is None:
            return None
        
        try:
            # Проверяем и нормализуем размерность
            if len(entity_features.shape) == 1:
                entity_features = entity_features.reshape(1, -1)
            
            # Если entity features имеют неправильную размерность, исправляем
            if entity_features.shape[1] != self.entity_feature_dim:
                ...
                # Дополняем нулями или обрезаем до нужной размерности
                if entity_features.shape[1] < self.entity_feature_dim:
    ...
    def _safe_hstack(self, arrays: List[np.ndarray]) -> np.ndarray:
        """Безопасное горизонтальное объединение массивов"""
        if not arrays:
            raise ValueError("Нет массивов для объединения")
        
        # Проверяем, что все массивы имеют одинаковое количество строк
    ...
    def prepare_categorical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Заполняем пропуски и возвращаем
    ...
    def get_feature_dimensions(self, 
        ...
        """
        Получение ожидаемой размерности финального вектора признаков
        
        Args:
            text_embedding_dim: Размерность текстовых эмбеддингов
            has_categorical: Включать ли категориальные признаки
            has_entity_features: Включать ли entity features
            
        Returns:
            Общая размерность признаков
        """
    ...
    def ensure_feature_dimension(self, features: np.ndarray) -> np.ndarray:
        """
        Корректировка размерности признаков для предсказания
        
        Args:
            features: Вектор признаков
            
        Returns:
            Вектор с правильной размерностью
        """
        if self.expected_feature_dim is None:
            return features
